...
- **Diagrams** for the system, the answer pipeline and statute search, as SVGs generated by
  `docs/diagrams.py`.
- **A logo.** A speech bubble (a plain-language answer) holding a § (the section it rests on),
  with a saffron bookmark ribbon (the citation that marks the place), on the app's green. Used
  for the app icon, the header, the sign-in page and the README; a simpler § and ribbon for the
  favicon, which stays legible at 16 px. `docs/logo.py` builds every asset from Libre Baskerville
  glyph outlines (SIL OFL), including a social-preview image.
