...
- **Diagrams** for the system, the answer pipeline and statute search, as SVGs generated by
  `docs/diagrams.py`.
- **A logo.** Balanced scales of justice (the law) inside a speech bubble (a plain-language
  answer), in the app's green and white with a saffron finial: a nod to India without any
  national emblem. The favicon keeps the scales without the bubble, so it stays legible at
  16 px. Used for the app icon, the header, the sign-in page and the README. `docs/logo.py`
  builds every asset, including light and dark README lockups and a social-preview image.
...
- The system diagram redrawn as a numbered sequence: question, checks before spending, the
  turn, what it uses, and the answer streaming back.
